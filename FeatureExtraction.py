...
class FeatureExtraction:
    ...
    def analyze(self, image):
        ...
        signal_features = self.analyze_traffic_signals(image, crosswalk_bbox)
    ...
    def detect_crosswalk_patterns(self, image, gray_image):
        ...
        features['crosswalk_line_count'] = 0
        features['crosswalk_detected'] = False
        features['crosswalk_confidence'] = 0.0
        if lines is not None and len(lines) > 3:
            ...
            for line in lines[:min(50, len(lines))]:
                ...
                # Near-horizontal lines (within ±15 degrees)
    ...
    def detect_pedestrian_signals(self, image, hsv_image):
        ...
        walk_mask = cv2.inRange(hsv_image, np.array([40, 100, 100]), np.array([80, 255, 255]))
        dont_walk_mask = cv2.inRange(hsv_image, np.array([0, 100, 100]), np.array([20, 255, 255]))
            ...
            if cv2.contourArea(contour) > 100:  # Minimum size
                ...
                if len(approx) == 4:
                    ...
                    if 0.5 < aspect_ratio < 2.0:
                        ...
                        roi_hsv = cv2.cvtColor(region_of_interest, cv2.COLOR_BGR2HSV)
    ...
    def analyze_pedestrian_blockage(self, image, gray_scale_image, crosswalk_bbox, crosswalk_mask):
        ...
        if crosswalk_mask is not None and crosswalk_mask.any():
            ...
            crosswalk_gray = cv2.cvtColor(crosswalk_area, cv2.COLOR_BGR2GRAY)
